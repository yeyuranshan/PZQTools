...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        toplabel.text = @"今日成长";
        toplabel.font = [UIFont systemFontWithName:@"PingFangSC-Regular" size:12];
        toplabel.textColor = [UIColor colorWithHexString:@"#4a4a4a"];
        ...
        [self.bottomLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(weakSelf.progressLabel.mas_bottom);
            make.centerX.equalTo(weakSelf.mas_centerX);
        }];
...
- (void)setGrowUp:(M_GrowUp *)growUp {
    _growUp = growUp;
    _progressLabel.text = [NSString stringWithFormat:@"%ld",(long)growUp.myTodayExp];
    if (growUp.myTodayExp >= growUp.myTodayTargetExp) {
        self.bottomLabel.text = @"点击右上角\n领取积分";
        self.bottomLabel.font = [UIFont systemFontWithName:@"PingFangSC-Regular" size:10.0];
        self.fullImageView.hidden = NO;
    }else{
        self.bottomLabel.text = [NSString stringWithFormat:@"目标：%ld点",(long)growUp.myTodayTargetExp];
        self.bottomLabel.font = [UIFont systemFontWithName:@"PingFangSC-Regular" size:12];
        self.fullImageView.hidden = YES;
    }
    self.progress = (CGFloat)growUp.myTodayExp / growUp.myTodayTargetExp;
    self.yHeight = self.bounds.size.height * (1 - self.progress);
...
{
    __weak typeof(self) weakSelf = self;
    self.timer = [CADisplayLink displayLinkWithTarget:weakSelf selector:@selector(waveAnimation)];
...
- (UIImageView *)fullImageView {
    if (!_fullImageView) {
       ...
       _fullImageView.image = [UIImage sd_animatedGIFNamed:@"ball"];
    }
...
-(UILabel *)progressLabel{
    if (!_progressLabel) {
        ...
        _progressLabel.font=[UIFont systemFontWithName:@"PingFangSC-Medium" size:36];
        _progressLabel.textColor= [UIColor colorWithHexString:@"#4A4A4A"];
...
- (UILabel *)bottomLabel {
    if (!_bottomLabel) {
        ...
        _bottomLabel.font = [UIFont systemFontWithName:@"PingFangSC-Regular" size:12];
        _bottomLabel.textColor = [UIColor colorWithHexString:@"#4a4a4a"];
    }
    return _bottomLabel;
}
